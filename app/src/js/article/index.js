...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const articleTmpl = document.querySelector(
    ".articles__item-tmpl"
  ).firstElementChild;
        ...
        if (statusCode == 200 && Array.isArray(data)) {
          ...
          data.forEach((article) => {
            ...
            frag
              ...
              .classList.add(`articles__item-${article.id}`);
            frag
              .querySelector(".articles__item")
              .setAttribute("href", `/articles/${article.id}`);
            frag.querySelector(".articles__item-title").textContent =
              article.title;
            frag.querySelector(".articles__item-date").textContent =
              article.created.split("T")[0]; //+年-月-日のみを抽出
